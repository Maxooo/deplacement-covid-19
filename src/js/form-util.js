...
import { addProfileToHistory, displayHistoryProfiles, getHistoryProfiles, removeProfileFromHistory } from './profile.js'
...
$('#profile-history').addEventListener('click', async event => {
  if (event.target.closest('.generate-attestation')) {
    ...
    const creationHour = creationInstant
      .toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })
      .replace(':', '-')
    profile.datesortie = creationInstant.toLocaleDateString()
    profile.heuresortie = creationInstant.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
    const pdfBlob = await generatePdf(profile, profile.reasons, pdfBase)
